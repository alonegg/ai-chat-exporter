class ChatExtractor {
  ...
  convertToMarkdown(conversation) {
    ...
    if (!conversation.messages || conversation.messages.length === 0) {
      ...
      markdown += `Please ensure:\n`;
      markdown += `- The conversation is fully loaded\n`;
      markdown += `- You are on a supported AI chat platform\n`;
      markdown += `- The page has finished loading completely\n\n`;
      markdown += `If the issue persists, please report it as a bug.\n`;
    } else {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'extractChat') {
    console.log('📨 AI Chat Exporter: Starting extraction...');
    
    try {
      const extractor = new ChatExtractor();
      const conversation = extractor.extractConversation();
      
      // If platform-specific extraction failed, try generic
      if (!conversation || conversation.messages.length === 0) {
        console.log('⚠️ Platform-specific extraction failed, trying generic method...');
        const genericConversation = extractor.extractGeneric();
        
        if (genericConversation && genericConversation.messages.length > 0) {
          const markdown = extractor.convertToMarkdown(genericConversation);
          console.log('✅ Generic extraction successful:', genericConversation.messages.length, 'messages');
          sendResponse({
            success: true,
      ...
      if (conversation && conversation.messages.length > 0) {
        ...
        sendResponse({
          success: false,
          error: 'Unable to extract conversation from this page. Please ensure the conversation is fully loaded and you are on a supported AI platform.'
        });
      }
    } catch (error) {
